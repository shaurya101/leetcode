...
// Keep a track of the current time time and total wait time waitingTime.
// If the time of arrival of the next customer is greater than the current time, fast forward to that time time = Math.max(cust[0],time)
// Increament the time with the time required to prepare the dish time = time + cust[1]
// Calculate the total time customer[i] had to wait and add it to total time.
// Divide by the total no. of customers to get average.
